...
void main() {
  group('Matrix Validation & Screen Block Tests', () {
    testWidgets('shows "Conecta el grafo para poder ver la matriz" when graph has disconnected nodes', (tester) async {
      ...
      expect(find.text('Conecta el grafo para poder ver la matriz'), findsOneWidget);
      expect(find.text('Existen nodos o secciones sin conectar en el lienzo.'), findsOneWidget);
      expect(find.byIcon(Icons.link_off_rounded), findsOneWidget);
    });

    testWidgets('renders matrix grid when graph is connected and valid', (tester) async {
      ...
      expect(find.text('Conecta el grafo para poder ver la matriz'), findsNothing);
